...
class RxSwiftViewController: UIViewController {
    ...
    let nameLabel: UILabel = UILabel.init(frame: CGRect.init(x: 20, y: 130, width: 100, height: 40))
...
//        Observable<String>.combineLatest(firstNameTextFielde.rx.text, lastNameTextFielde.rx.text){
//            "\($0) \($1)"
//        }.map { "Greetings, \($0)" }.bind(to: nameLabel.rx.text)
//        
//        (firstNameTextFielde.rx.text, lastNameTextFielde.rx.text) {return  }.map{"Greetings, \($0)"}.bind(to:nameLabel.rx.text)
